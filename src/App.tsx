  ...
  <div className="flex flex-col mb-4 pt-4 animate-enter shrink-0 relative z-50">
    <div className="flex justify-between items-start">
      ...
      <div className="flex gap-2 items-center">
        {aiActive && (
          <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-teal-500/10 border border-teal-500/20 mr-1 animate-pulse">
            <Sparkles size={10} className="text-teal-400" />
            <span className="text-[8px] font-bold text-teal-400 uppercase tracking-wider">Live AI</span>
          </div>
        )}
        {!isDashboard && (
...
const Integration: React.FC<IntegrationProps> = ({ goal, setGoal, goalStep, setGoalStep, isLocked, setIsLocked, expandingBelief, stressor, sessionCount, completeSession, resetApp, setView, toggleSound, soundEnabled, somaticZones, isBurnoutPath }) => {
  ...
  useEffect(() => {
    if (isLocked && !isBurnoutPath && !manifesto) {
      setGenerating(true);
      generateManifesto(stressor, expandingBelief, goal.action).then(res => {
