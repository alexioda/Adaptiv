...
const Horizon: React.FC<HorizonProps> = ({
  ...
  const startAIConversation = async () => {
    ...
    patchHorizon({ chatHistory: [{ role: 'ai', text: firstQ, isHtml: false }], showChatInput: true });
    setIsTyping(false);
  };
  ...
  const sendUserMessage = async () => {
    ...
    if (newCount < 3) {
      ...
      patchHorizon(h => ({ chatHistory: [...h.chatHistory, { role: 'ai', text: nextQ, isHtml: false }], showChatInput: true }));
    } else {
      const validationHtml = await generateHorizonValidation(stressor, perception, historyText);
      patchHorizon(h => ({ chatHistory: [...h.chatHistory, { role: 'ai', text: validationHtml, isHtml: true }] }));
...
const Integration: React.FC<IntegrationProps> = ({
  ...
  onBack, saveSession, updateLatestSession, setHasCompletedFreeCycle, goHome
}: any) => {
  ...
  useEffect(() => {
    ...
    generateManifesto(stressor, truth, action, fear, entryLevel, (text) => setManifesto(text))
