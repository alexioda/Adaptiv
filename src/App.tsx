...
const analyzeCurrentEnergy = async (
  ...
  try {
    ...
    const prompt = `Act as an expert Energy Leadership Master Practitioner (ELI-MP). Analyze the client's state.
...
2. Write a compassionate, direct, 2-sentence "Reflection" mirror statement.

Constraints: Use "You" statements. NO ELI jargon.
...
Context: Situation: ${stressor} | Experience: ${perception} | Body: ${somatic}
...
Guidelines: First person ("I"). Under 50 words. No jargon. Output ONLY the text.`;
    ...
    const fallback = `I observe the heavy energy of "${stressor}" and the quiet fear that ${fear || 'I am not enough'}. I honor it, but I do not reside there. I choose to shift. Standing in the truth that ${truth}, I reclaim my power. My action is my seal: ${action}.`;
...
Client's issue: "${stressor}" | Feeling: "${perception}" | Context: ${historyText}

Ask ONE short, profound, open-ended coaching question (under 20 words) to uncover root friction.`;
...
Deliver a final empathetic validation in 3 parts using HTML spans:

1. <span class="block mb-2 font-bold text-teal-300">I hear you.</span>

2. <span class="block mb-4 text-white/80 italic">Validate their specific experience compassionately.</span>

3. <span class="block font-bold text-white">We can clear this static and reclaim your bandwidth. To shift this, we need to locate it.</span>

Return ONLY the raw HTML. No markdown.`;

    const raw = (await callAI(prompt)).trim().replace(/```html|```/g, '').trim();

    return sanitizeHtml(raw);   // ← sanitize before use

  } catch {

    return sanitizeHtml(`<span class="block mb-2 font-bold text-teal-300">I hear you.</span><span class="block mb-4 text-white/80 italic">It makes complete sense that you feel this way. You have been running on empty, and that takes a massive toll on your system.</span><span class="block font-bold text-white">We can clear this static and reclaim your bandwidth. To shift this, we need to locate it.</span>`);
...
// NEW: Pattern analysis across sessions
...
const generatePatternInsight = async (history: SessionRecord[]): Promise<string> => {
  ...
  try {
    ...
    const prompt = `You are an expert transformational coach reviewing a client's recent session history.

${summary}

In 1-2 sentences, identify the single most important pattern you notice across these sessions.

Be direct, compassionate, and specific. No jargon. Start with "I notice..."`;
...
// NEW: Assess exit energy level after session completion
...
const assessExitEnergy = async (
  ...
  try {
    ...
    const prompt = `Act as an expert Energy Leadership Master Practitioner (ELI-MP).

A client just completed a stress transformation session. Assess their exit energy level.


Entry context: They were operating at Level ${entryLevel} when they began.
...
- New truth / expanding belief: "${expandingBelief}"

- Committed action: "${commitment}"

- Self-rated post-session stress (1-10): ${postStress}

- Self-rated post-session energy (1-10): ${postEnergy}


ELI Scale reference:

Level 1: Victim — "I lose." Catabolic. Stuck, no agency.

Level 2: Fighter — "I win, you lose." Catabolic. Conflict-driven.

Level 3: Rationalizer — "I cope." Mild anabolic. Tolerating.

Level 4: Caregiver — "You win." Anabolic. Compassion-driven.

Level 5: Opportunist — "We both win." Anabolic. Opportunity-focused.

Level 6: Visionary — "Everyone wins." High anabolic. Intuition-led.

Level 7: Creator — "I create." Pure anabolic. Purpose-aligned.


Task: Based on the quality of their expanding belief, the specificity of their commitment, and their post-session self-ratings, identify the most likely exit energy level (1-7).


Important: The exit level should be equal to or higher than the entry level. Only score lower if the post-session stress is significantly higher than would suggest shift.


Return ONLY a raw JSON object: { "level": number, "rationale": "one sentence" }`;
    ...
    // Graceful fallback — use stress delta heuristic

    const stressDelta = postStress - Math.round((postEnergy / 10) * 10);
