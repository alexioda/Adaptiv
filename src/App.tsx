...
const Horizon: React.FC<HorizonProps> = ({ userName, sessionCount, stressor, setStressor, perception, setPerception, setView, toggleSound, soundEnabled, resetApp, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      <Nav title={`Hello, ${userName || 'Traveler'}`} subtitle={`Session ${sessionCount + 1}`} onBack={handleInternalBack} isDashboard={step === 'intake'} resetApp={resetApp} toggleSound={toggleSound} soundEnabled={soundEnabled} progress={step === 'intake' ? 10 : step === 'chat' ? 20 : 30} />
          ...
          <div className="glass-panel p-6 rounded-[2rem] border border-white/10 shadow-sm flex flex-col h-[70vh] animate-[slideUpFade_0.5s_ease-out_forwards]">
            ...
            {showRouteButton && (
              <button 
                ...
                className="w-full mt-4 py-4 bg-teal-500 text-slate-900 font-bold rounded-xl text-[10px] uppercase tracking-widest text-center shadow-[0_0_20px_rgba(20,184,166,0.3)] hover:bg-teal-400 transition-all animate-[slideUpFade_0.5s_ease-out_forwards]"
              >
                Locate the Friction &rarr;
              </button>
            )}
          </div>
        )}
...
const Breath: React.FC<BreathProps> = ({ breathing, setBreathing, breathCount, setBreathCount, setView, toggleSound, soundEnabled, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col justify-center items-center">
      <Nav title="Regulation" subtitle="Breathe" onBack={onBack} toggleSound={toggleSound} soundEnabled={soundEnabled} />
...
const Alchemy: React.FC<AlchemyProps> = ({ setView, toggleSound, soundEnabled, onBack }) => (
  <div className="h-full flex flex-col">
...
const Integration: React.FC<IntegrationProps> = ({ goal, setGoal, goalStep, setGoalStep, isLocked, setIsLocked, expandingBelief, stressor, fear, sessionCount, completeSession, resetApp, setView, toggleSound, soundEnabled, somaticZones, isBurnoutPath, userName, energyAnalysis, stressLevel, energyLevel, postStressLevel, setPostStressLevel, postEnergyLevel, setPostEnergyLevel, onBack }: any) => {
...
const EnergyAnalyzer: React.FC<EnergyAnalyzerProps> = ({ setView, onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col justify-center animate-enter">
      <Nav title="Energy Lens" subtitle={`Question ${step + 1} / 6`} onBack={onBack} toggleSound={() => {}} soundEnabled={false} progress={((step + 1) / 6) * 100} />
    ...
    <>
      ...
      <div className="fixed inset-0 bg-slate-950 text-white font-sans overflow-hidden flex justify-center">
        ...
        <div className="w-full max-w-md h-full relative z-10 p-6">
          ...
          {view === 'energy_reflection' && <EnergyReflection energyAnalysis={energyAnalysis} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('dashboard')} />}
          
          {view === 'preservation' && <Preservation setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} setGoal={setGoal} setExpandingBelief={setExpandingBelief} setViewToIntegration={() => { setIsLocked(true); setView('integration'); }} onBack={() => setView('dashboard')} />}
          
          {view === 'burnout_check' && <VitalityScan setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} setBurnoutPath={setIsBurnoutPath} onBack={() => setView('dashboard')} />}
          
          {view === 'fork_entry' && <ForkEntry setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('dashboard')} />}
          
          {view === 'diffuser' && <Diffuser fear={fear} setFear={setFear} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('fork_entry')} />}
          
          {view === 'somatic' && <Vessel somaticZones={somaticZones} setSomaticZones={setSomaticZones} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('dashboard')} />}
          
          {view === 'partswork' && <PartsWork selectedPart={somaticZones[0] || 'Part'} sensation={sensation} setSensation={setSensation} protection={protection} setProtection={setProtection} fear={fear} setFear={setFear} expandingBelief={expandingBelief} setExpandingBelief={setExpandingBelief} partsStep={partsStep} setPartsStep={setPartsStep} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('somatic')} />}
          
          {view === 'laser' && <LaserCoaching stressor={stressor} perception={perception} somatic={somaticZones[0] || 'Body'} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} setGoal={setGoal} setExpandingBelief={setExpandingBelief} energyLevel={energyLevel} stressLevel={stressLevel} onBack={() => setView('dashboard')} />}
          
          {view === 'lens' && <Perspective pressure={pressure} setPressure={setPressure} ability={ability} setAbility={setAbility} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('dashboard')} />}
          
          {view === 'fork' && <Crossroads stressLevel={stressLevel} energyLevel={energyLevel} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('lens')} />}
          
          {view === 'regulate' && <Breath breathing={breathing} setBreathing={setBreathing} breathCount={breathCount} setBreathCount={setBreathCount} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('fork')} />}
          ...
          {view === 'insight' && <Insight expandingBelief={expandingBelief} setExpandingBelief={setExpandingBelief} setView={setView} toggleSound={toggleSound} soundEnabled={soundEnabled} onBack={() => setView('regulate')} />}
